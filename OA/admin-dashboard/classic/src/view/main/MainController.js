Ext.define('Admin.view.main.MainController', {
    ...
    init:function(){
    	Ext.Ajax.request({
            url: 'http://localhost:8080/employee/getLoginInfo',
            method: 'post',
            success: function(response, options) {
            	console.log(response);
            	var json = Ext.util.JSON.decode(response.responseText);
            	var position = Ext.getCmp("userPositon").setText(json.position);
            	console.log(position);
            	var loginUserName = Ext.getCmp("loginUserName").setText("用户名： "+json.name);;
            	var headIcon = Ext.getCmp("head_Icon");
            	var path = '/images/employee/' + json.picture;
		        headIcon.setSrc(path);
            }
        });
    }
