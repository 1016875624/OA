...
删除假条（删除状态，只能删除待申请和已销假状态的假条）

修改假条（待申请状态，只能修改待申请状态的假条）

查询假条（可根据请假开始和结束时间或假条状态来查询假条，也可以结合两者一起进行高级查询）

发起请假（待审批状态，同时会发生邮件提醒上级）

#### 具体实现

先根据session查询出当前登录员工的所有假条，员工可以对自己的假条进行增删查改，员工刚新建出来的假条是待申请状态，员工可以发起假条审批，状态变为待审批状态，然后系统根据数据库中员工表的上级进行申请请假并利用封装的mail工具类发送到当前员工上级的邮箱里，如果上级看过邮箱内容后同意请假，即审批通过，状态变为审批通过状态，不同意即审批不通过，不同意理由将会覆盖原来的请假理由并变为被驳回状态，修改过后即可重新申请请假。另外用户可以根据假条的开始结束时间段以及假条状态进行查询或是高级查询。
...
题库管理做的就是可以在前端进行增加、修改、查询、删除的操作。因为题目不可能在数据库中进行操作的
...
出卷要求3条选择题3条多选题4条填空题，每题10分，全部答对才能给分，答对一个，少答不得分。
...
评分这里应该要做到能够忽略大小写，多选题忽略顺序，进行比对答案。
...
![工时模块用例图](img\hxd\1.png)

### 题库模块用例图

![题库模块用例图](img\hxd\3.png)

### 答题模块用例图

![答题模块用例图](img\hxd\4.png)
